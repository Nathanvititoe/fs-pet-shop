...
let pets = JSON.parse(fs.readFileSync('pets.json', 'utf-8'));
let command = process.argv.slice(2);
let index = command[1];

if (command.length === 1 && command[0] === 'read') {
    console.log(pets);
} else if (index < 0 || index >= pets.length ) {
        console.error(`Usage: node pets.js read INDEX`);
        process.exit(1);
    } 
 //   if(pets[command[1]] === undefined){
        console.log()
    console.log(pets[command[1]])
//}

if (command.length === 1 && command === 'create') {
    create ();
}
...
function create () {
    ...
    if (!age || !kind || !name) {
        ...
        const petobj = { age: parseInt(age), kind: kind, name: name};
    }
}
*/
